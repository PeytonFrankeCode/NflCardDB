...
export async function onRequestGet({ request, env }) {
  ...
  // Best offers included: on those the price is the seller's ask rather than
  // what was paid, so `is_ask` below tells the page which are which.
  const where = ["price_cents IS NOT NULL"];
  const binds = [];
  ...
  return json({
    sales: (rows.results || []).map((r) => ({
      ...
      // True when that price is an asking price and the card went for less.
      is_ask: r.ask_cents != null,
      player: r.player,
