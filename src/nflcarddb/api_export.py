...
def _insert_batches(table: str, columns, rows, pk: str):
    """Yield one INSERT per batch of rows, rather than building them all first.

    A generator on purpose. Half a million sales is roughly 365 MB of SQL, and
    accumulating it in a list, joining it into one string and then encoding
    that to bytes held three copies at once -- which is how a restore-then-push
    ran a modest PC out of memory on the one upload that mattered.
    """
    cols = ", ".join(columns)
    ...
    batch: list[str] = []
    for row in rows:
        batch.append("(" + ", ".join(_sql_literal(row[c]) for c in columns) + ")")
...
DAILY_COLUMNS = ("sold_date", "sales", "priced", "median_cents", "p90_cents",
                 "total_cents")
...
def write_sql(
    ...
    """Write the D1 import script straight to a file. Returns stats.

    Sales are streamed from the cursor to the file, so the largest thing held
    at once is one batch. The card keys the upload touches are collected on the
    way past, which is why the rollups are written afterwards -- they need to
    know what moved, and reading the sales twice to find out would cost the
    saving.
    """
    ...
    counts = {"rows": 0, "days": 0, "cards": 0, "card_grades": 0}
    touched: set[str] = set()

    try:
        with out.open("w", encoding="utf-8") as handle:
            handle.write("-- Generated by `nflcarddb export-api`. Safe to re-run:\n")
            handle.write("-- sales upsert on item_id, so a re-import updates "
                         "rather than duplicates.\n")

            def sales_rows():
                for row in _iter_rows_to_export(conn, since, changed_since):
                    counts["rows"] += 1
                    if row["card_key"]:
                        touched.add(row["card_key"])
                    yield row

            for statement in _insert_batches("sales", EXPORT_COLUMNS,
                                             sales_rows(), "item_id"):
                handle.write(statement + "\n")

            # Recomputed in full regardless: there is one row per day, so ~90 of
            # them, and a partial day's medians would be wrong.
            rollups = _daily_rollups(conn, since)
            counts["days"] = len(rollups)
            for statement in _insert_batches("daily", DAILY_COLUMNS, rollups,
                                             "sold_date"):
                handle.write(statement + "\n")

            # Only the cards this upload touches, on an incremental run. A card
            # whose sales did not change has the stats it had last time, and
            # there are tens of thousands of them.
            cards, card_grades = _card_rollups(
                conn, touched if changed_since else None)
            counts["cards"] = len(cards)
            counts["card_grades"] = len(card_grades)
            for table, columns, payload, pk in (
                ("cards", CARD_COLUMNS, cards, "card_key"),
                ("card_grades", GRADE_COLUMNS, card_grades,
                 "card_key, grade_label"),
            ):
                for statement in _insert_batches(table, columns, payload, pk):
                    handle.write(statement + "\n")

            keys = list(key_hashes or [])
            for key_hash, label in keys:
                handle.write(
                    "INSERT INTO api_keys (key_hash, label, created_at) VALUES "
                    f"({_sql_literal(key_hash)}, {_sql_literal(label)}, "
                    f"{_sql_literal(now)}) "
    ...
    return {**counts, "since": since, "changed_since": changed_since,
            "keys_added": len(list(key_hashes or [])), "generated_at": now,
            # The highest timestamp in the database, which becomes the next
...
def build_sql(
    ...
    """The script as one string. Convenient for tests and small exports.

    Written through the file path rather than beside it, so there is one
    implementation and the streaming version cannot drift away from the one
    the tests exercise.
    """
    import tempfile

    with tempfile.TemporaryDirectory() as tmp:
        path = Path(tmp) / "export.sql"
        stats = write_sql(db_path, path, since, key_hashes, changed_since)
...
def export_api_sql(
    ...
    stats = write_sql(db_path, out_path, since, list(key_hashes or []),
                      changed_since)
    out = Path(out_path)
    stats["file"] = str(out)
